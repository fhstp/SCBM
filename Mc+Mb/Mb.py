...
if __name__ ==  '__main__':
    ...
    for i in range(5):
        ...
        print(class_map)
        _ = train_model_dev(data_train=train, data_dev=test, epoches=16, batch_size=32, #! lr set to 2e-6
                        interm_layer_size = 128, lr =  2e-6 ,  decay=1e-6, output='.', task='Class', 
                        classes_len = len(Counter(train['Class']).keys()), output_name = "Mb-large-germeval",
        ...
        model.load(f"Mb-large-germeval.pt")
    ...
    import pickle
    with open('../germeval/Mb-large.pickle', 'wb') as handle:
