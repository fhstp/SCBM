...
class SeqModel(torch.nn.Module):
  ...
  def __init__(self, interm_size, classes_len):
    ...
    self.transformer, self.tokenizer = HugginFaceLoad( "FacebookAI/xlm-roberta-large" )
    ...
    self.condense_feature = torch.nn.Sequential(torch.nn.Linear(in_features=244, out_features=768))
    self.intermediate_plus = torch.nn.Sequential(
                                            torch.nn.Linear(in_features= 1792, out_features=self.interm_neurons),
                                            torch.nn.LeakyReLU(),
  ...
  def forward(self, data, get_mask = False):
    ...
    feat = feat*mask
    feat = self.condense_feature(feat)
    X = torch.concat((X, feat), dim=-1)
    enc = self.intermediate_plus(X)
...
if __name__ ==  '__main__':
    ...
    train_file_name = '../tsnh/TSNH_uniform.csv'
    ...
    train = train[train['id'].isin(list(mapping_train.keys()))]
    ...
    for i, (train_index, test_index) in enumerate(skf.split(train['text'], train['Class'])):
        ...
        _ = train_model_dev(data_train=train.iloc[train_index], data_dev=train.iloc[test_index], epoches=16, batch_size=32,#! lr set to 2e-6
                        interm_layer_size = 128, lr = 2e-6,  decay=1e-6, output='.', task='Class', 
                        classes_len = len(Counter(train['Class']).keys()), output_name = "Mb-tsnh-large",
        ...
        model.load(f"Mb-tsnh-large.pt")

        test_performance += [evaluate(model, task = 'Class', data_dev = train.iloc[test_index], bs = 8, 
                                        mapping_feat = {'train': mapping_train, 'test' : mapping_train},
                                        class_map = class_map)]
    ...
    import pickle
    with open('../tsnh/Mb.pickle-large', 'wb') as handle:
