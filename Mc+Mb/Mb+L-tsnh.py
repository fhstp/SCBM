...
class SeqModel(torch.nn.Module):
  ...
  def __init__(self, interm_size, classes_len):
    ...
    self.intermediate_plus = torch.nn.Sequential(
                                            torch.nn.Linear(in_features= 768*2, out_features=self.interm_neurons),
                                            torch.nn.LeakyReLU(),
...
if __name__ ==  '__main__':
    ...
    with open(f'{train_file_name}.pickle-ll2', 'rb') as handle:
    ...
    for i, (train_index, test_index) in enumerate(skf.split(train['text'], train['Class'])):
        ...
        _ = train_model_dev(data_train=train.iloc[train_index], data_dev=train.iloc[test_index], epoches=16, batch_size=32,
                        interm_layer_size = 128, lr = 1e-5,  decay=1e-6, output='.', task='Class', 
                        classes_len = len(Counter(train['Class']).keys()), output_name = "Mb+L-tsnh-ll2",
    ...
    import pickle
    with open('../tsnh/Mb+L.pickle-ll2', 'wb') as handle:
