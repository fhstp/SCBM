...
X_train, X_test, y_train, y_test = train_test_split(train['id'], train['Class'], test_size=0.2, random_state=42)
test = train[train['id'].isin(X_test)]
train = train[train['id'].isin(X_train)]
...
for _, row in train.iterrows():
    if row['id'] not in mapping_train:
        print(row['id'])
        continue
...
for _, row in test.iterrows():
    feature_vectors_test += [mapping_train[row['id']]]
...
for feature_size in tqdm(sorted(list( set(range(1, 244, 8)) | {244}))):
	...
	for observation in range(30):
		...
		_ = train_model_dev(data_train=train, data_dev=test, epoches=100, batch_size=128,
                        interm_layer_size = 128, lr = 2e-3,  decay=1e-6, output='weights_tsnh', task='Class', 
		...
		model.load(f"weights_tsnh/fs={feature_size}.pt", verbose=False)

		test_performance = evaluate(model, task = 'Class', data_dev = test, bs = 128, 
									mapping_feat = {'train': mapping_train, 'test' : mapping_train},
										class_map = class_map, feature_mask=active_features, 
										verbose = False)

		train_performance = evaluate(model, task = 'Class', data_dev = train, bs = 128, 
									mapping_feat = {'test': mapping_train, 'train' : mapping_train},
										class_map = class_map, feature_mask=active_features, 
										verbose = False)
		del model
	...
	with open(f'performance_tsnh.pickle', 'wb') as handle:
