...
class LossFunction(torch.nn.Module):
  ...
  def forward(self, outputs, labels, masks = None):
    ...
    if masks is not None:
      ...
      avg_mask = torch.stack([masks[i].mean( dim = 0) for i in z if len(i[0]) >= 1])
      
      #pairwise product of the mask
      dot_products = torch.mm(avg_mask, avg_mask.T)  # Shape (n, n)
      mask_loss = (dot_products.sum() - torch.diag(dot_products).sum()) / 2.0
...
if __name__ ==  '__main__':
    ...
    for i in range(5):
        ...
        del model
    import pickle
    with open('../hs_cs/Mc+L.pickle', 'wb') as handle:
